...
import type { FaqComposeReply, FaqComposeRequest, FaqTicket } from "@4pm/ws";
...
function buildPrompt(tickets: FaqTicket[]): string {
...
export async function runFaqCompose(req: FaqComposeRequest, ai: FaqComposeAi): Promise<FaqComposeReply> {
  ...
  try {
    ...
    const usage = await runClaudeWithFailover(buildPrompt(req.tickets), workDir, ai);
    ...
    if (!staged.stdout.trim()) return { prUrl: "", tokens: usage.tokens || estimateTokens(""), finishedAt: new Date().toISOString() };
